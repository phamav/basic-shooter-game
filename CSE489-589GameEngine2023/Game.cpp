...
void Game::processGameInput()
{
	...
	float elapsedTime = std::chrono::duration<float>(currentTime - gameStartTime).count();

	if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_ESCAPE) || elapsedTime >= gameDuration) {

		isRunning = false;
	}

	if (glfwGetKey(glfwGetCurrentContext(), GLFW_KEY_F) && WireFrame_KeyDown == false) {
